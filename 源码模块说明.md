...
- **内置本地助手**（Windows 桌面版）：llama.cpp 双后端由主进程托管（`electron-ai-engine.cjs`），默认模型 MiniCPM5-2B-Q4_K_M.gguf（约 1.5 GB，另保留 Q8_0 约 2.5 GB）按需下载；模型目录内所有 GGUF 均可选用运行，启动时自动拉起；🔴🟢 圆点显示引擎状态
- **外部 llama-server**（高级）：手动对接任意 OpenAI 兼容本地服务（原「本地 LLM」模式）
